%INTERO_BOOTSTRAP_CORR permutation test of association for linear or
%circular data (in radians)
%
%   usage: [pval, stats] = intero_bootstrap_corr(DV1, type1, DV2, type2, [optional])
%
%    INPUTS:
...
%        permutations, call:
%        [pval, stats] = intero_bootstrap_corr( age, 'linear, memory, 'linear, 5000, 'pearson')
%
%        2. For a circular-linear correlation between the phase of the
%           cardiac cycle at stimulus onset (in radians) and memory score,
%           call:
%           [pval, stats] = intero_bootstrap_corr( cardiac, 'circular',
%           memory, 'linear' );
% ========================================================================
%  INTERO TOOLBOX v1.0
...
function [pval,stats] = intero_bootstrap_corr(DV1, type1, DV2, type2, varargin)
...
assert(numel(DV1)>2,'Error in <strong>intero_bootstrap</strong>: need at least 3 datapoints in DV1');
assert(numel(DV2)>2,'Error in <strong>intero_bootstrap</strong>: need at least 3 datapoints in DV2');

% check DV1 and DV2 are nx1 or 1xn
assert(min(size(DV1))==1,'Error in <strong>intero_bootstrap</strong>: DV1 should be 1xn or nx1');
assert(min(size(DV2))==1,'Error in <strong>intero_bootstrap</strong>: DV2 should be 1xn or nx1');

% check DV1 and DV2 are 'cicular' or 'linear'
assert(ismember(type1,{'circular','linear','Circular','Linear'}),'Error in <strong>intero_bootstrap</strong>: type1 should be linear or circular');
assert(ismember(type2,{'circular','linear','Circular','Linear'}),'Error in <strong>intero_bootstrap</strong>: type2 should be linear or cicular');
...
assert(numel(DV1)==numel(DV2),'Error in <strong>intero_bootstrap</strong>: DV1 and DV2 should have the same number of datapoints.');
...
for i = 1:numel(varargin)
    ...
    if ischar(varargin{i}) & ...
        ...
        warning(['Warning in <strong>intero_bootstrap<\strong>: input ' varargin{i} ' is unknown. Ignoring...']);
...
str = 'Warning in <strong>intero_bootstrap<\strong>: Your data look like degrees, not radians. Please check and re-run if neccessary';
