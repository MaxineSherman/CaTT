%INTERO_BOOTSTRAP_DIFF permutation test of difference
%   usage: [pval, stats] = intero_bootstrap_diff(group1, group2, design, [optional])
%
%    INPUTS:
...
%        [pval, stats] = intero_bootstrap_diff( patients, controls, 'between')
%
%        2. If you predict patients > controls and want a one-tailed test, call:
%        [pval, stats] = intero_bootstrap_diff( patients, controls, 'between', 'higher')
%
...
%          [pval, stats] = intero_bootstrap_diff( correct,...
%                                               incorrect, ...
%                                               'within', ...
%                                               'median', ...
%                                               'circular');
...
function [pval,stats] = intero_bootstrap_diff(group1, group2, design, varargin )
...
assert(numel(group1)>2,'Error in <strong>intero_bootstrap</strong>: need at least 3 datapoints in group 1');
assert(numel(group2)>2,'Error in <strong>intero_bootstrap</strong>: need at least 3 datapoints in group 2');

% check group1 and group2 are nx1 or 1xn
assert(min(size(group1))==1,'Error in <strong>intero_bootstrap</strong>: group1 should be 1xn or nx1');
assert(min(size(group1))==1,'Error in <strong>intero_bootstrap</strong>: group2 should be 1xn or nx1');
...
elseif strcmpi(design,'within')
    assert(numel(group1)==numel(group2),'Error in <strong>intero_bootstrap</strong>: group1 and group2 should have the same number of datapoints for a Within-subjects design.');
else
    error('Error in <strong>intero_bootstrap</strong>: unknown design. Please enter within or between');
end
...
if strcmpi(stats.opt.data_type,'circular')
    ...
    if max( abs(group1) ) > 12 | max( abs(group2) ) > 12
        warning('Warning in <strong>intero_bootstrap<\strong>: Your data look like degrees, not radians. Converting...');
        group1 = circ_ang2rad( group1 );
        group2 = circ_ang2rad( group2 );
    end
