...
% unzip and move the filders to `data_Galvez-Pol`
...
for i = 1:numel(data_files)
    ...
    % get separate structures for saccades & fixations

    idx = 1:size(Saccades_Mx3,1);
...
for i = 1:numel(data_files)
    ...
    idx = 1:size(Saccades_Mx2,1);

    blinks(i).catt.IBI      = Saccades_Mx2(idx,3);
    blinks(i).catt.all_IBI  = num2cell(Saccades_Mx2(idx,3));
    blinks(i).catt.onsets   = Saccades_Mx2(idx,2);
...
for i_analysis = 1:3
    switch i_analysis
        ...
        case 3; group = blinks;    analysis_name = 'blinks';
    end
