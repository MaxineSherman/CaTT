...
function catt_demo_GalvezPol_twav
...
for i = 1:numel(data_files)
    ...
    [~, stats, group_twav(i).catt] = catt_bootstrap_clust( group(i).catt, 'rao', 1000 );
...
disp(['===============================================']);
disp(['% Group stats:' analysisName]);
disp(['===============================================']);

[Z_group,P_group] = catt_z2p( group_twav );
disp(sprintf('wrapping to t-wave (permutation): Group zval = %.3f, group pval = %.3f',[Z_group,P_group]));
...
print('-dpng',['figures/Fig4A_twav_' analysisName]);
print('-depsc',['figures/Fig4A_twav_' analysisName]);

%% run consistency
output = catt_consistency(group_twav);

save(['data_Galvez-Pol/twav_' analysisName]);
